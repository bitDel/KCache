...
#include "ArcCache/arc_cache.hpp"
...
private: std::chrono::time_point<std::chrono::high_resolution_clock> start_;
};
...
void testHotDataAccess() {
  ...
  kcache::LruCache<int, std::string> lru(CAPACITY);
  kcache::LfuCache<int, std::string> lfu(CAPACITY);
  kcache::ArcCache<int, std::string> arc(CAPACITY);

  kcache::LruKCache<int, std::string> lruk(CAPACITY, HOT_KEYS + COLD_KEYS, 2);
  kcache::LfuCache<int, std::string> lfuAging(CAPACITY, 20000);

  std::random_device rd;
  std::mt19937 gen(rd());

  std::array<kcache::CachePolicy<int, std::string>*, 5> caches = {&lru, &lfu, &arc, &lruk, &lfuAging};
...
void testLoopPattern() {
  ...
  kcache::LfuCache<int, std::string> lfu(CAPACITY);
  kcache::ArcCache<int, std::string> arc(CAPACITY);
  kcache::LruKCache<int, std::string> lruk(CAPACITY, LOOP_SIZE * 2, 2);
  kcache::LfuCache<int, std::string> lfuAging(CAPACITY, 3000);
...
void testWorkloadShift() {
  ...
  kcache::LfuCache<int, std::string> lfu(CAPACITY);
  kcache::ArcCache<int, std::string> arc(CAPACITY);
  kcache::LruKCache<int, std::string> lruk(CAPACITY, 500, 2);
  kcache::LfuCache<int, std::string> lfuAging(CAPACITY, 10000);
